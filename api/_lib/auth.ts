...
function firma(valor: string, secreto: string): string {
  return createHmac("sha256", secreto).update(valor).digest("base64url");
}

export function coincideSecreto(recibido: string, esperado: string): boolean {
...
export function crearSesion(secreto: string, ahora = Date.now(), minutos = 60 * 24 * 14): string {
...
export function leerSesion(token: string | undefined, secreto: string, ahora = Date.now()): boolean {
